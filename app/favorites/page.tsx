  ...
  if (!isLoggedIn) {
    return (
      <div className="grow flex flex-col items-center justify-center min-h-[60vh] bg-zinc-950 text-white px-6 text-center">
        ...
        <h1 className="text-2xl font-bold tracking-tight text-white mb-2">Your Favorites</h1>
        <p className="text-zinc-400 text-sm max-w-md mb-6">
          Sign in to view and save your favorite movies and TV series.
        </p>
        <Button
          onClick={openAuth}
  ...
  return (
    <div className="grow bg-zinc-950 text-white min-h-[80vh] py-24 px-6 sm:px-12 md:px-16 lg:px-20 max-w-7xl mx-auto w-full">
      <div className="flex items-center gap-3 border-b border-zinc-900 pb-6 mb-8">
        ...
        <div>
          <h1 className="text-2xl sm:text-3xl font-black tracking-tight text-white">My Favorites</h1>
          <p className="text-zinc-500 text-xs mt-0.5">
            {favorites ? favorites.length : 0} title{favorites?.length !== 1 ? "s" : ""} saved
          </p>
        </div>
      </div>
      ...
      {!favorites ? (
        ...
        <div className="text-center py-24 bg-zinc-900/10 border border-zinc-900 rounded-3xl p-8 flex flex-col items-center justify-center gap-3">
          ...
          <p className="text-zinc-400 text-sm font-medium">Your favorites list is currently empty.</p>
          <p className="text-zinc-600 text-xs">Explore the catalog and click the heart icon on any card to add it here.</p>
        </div>
      ) : (
        <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 xl:grid-cols-6 gap-4 sm:gap-5">
          {favorites.map((item) => {
            // Adapt Convex favorites database record format to TMDBMedia interface format for the Card
